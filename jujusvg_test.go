...
func (s *newSuite) TestNewFromBundle(c *gc.C) {
	...
	cvs.Marshal(&buf)
	c.Logf("%s", buf.String())
	assertXMLEqual(c, buf.Bytes(), []byte(fmt.Sprintf(`
<?xml version="1.0"?>
<!-- Generated by SVGo -->
...
func (s *newSuite) TestWithFetcher(c *gc.C) {
	...
	cvs, err := NewFromBundle(b, iconURL, true, new(emptyFetcher))
	c.Assert(err, gc.IsNil)
...
func (s *newSuite) TestDefaultFetcher(c *gc.C) {
	...
	cvs, err := NewFromBundle(b, tsIconUrl, true, nil)
	c.Assert(err, gc.IsNil)
...
func (s *newSuite) TestFetcherError(c *gc.C) {
	...
	_, err = NewFromBundle(b, iconURL, true, new(errFetcher))
